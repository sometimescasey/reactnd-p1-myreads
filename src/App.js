...
class BooksApp extends Component {
  ...
  changeBookShelf = (bookIdToAlter, newShelf) => {
    console.log("changeBookShelf called,", bookIdToAlter, newShelf);
    /* TODO: find out what bookIdToAlter and newShelf should be - names or what?
      Mock bookIdToAlter as title for now, change later

      TODO: this is very inefficient
      TOOD: needs to validate that newShelf value is good
    */
    const makeUpdatedState = (cs) => {
      let updatedShelvedBooks;
      let allOther = cs.shelvedBooks.filter((b) => b.title !== bookIdToAlter);
      if (newShelf !== "none") {
        /* TODO: assert check that this only returns a single item */
        let thisBook = cs.shelvedBooks.filter((b) => b.title === bookIdToAlter)[0];
        updatedShelvedBooks = allOther.concat([{title: thisBook.title, authors: thisBook.authors, coverURL: thisBook.coverURL, shelf: newShelf}]);
      } else {
        updatedShelvedBooks = allOther;
      }
      ...
      <div className="app">
        <Route exact path="/" render={() => (
          <Main
            ...
            changeSearchState={() => {this.setState({ showSearchPage: true })}} />
            )}/>
